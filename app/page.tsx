...
import { LayoutDashboard, ReceiptText, Stethoscope, Workflow, Building2, HeartHandshake, Bell, Upload, AlertCircle, FileText, Database, Search, ChevronRight } from 'lucide-react';
...
export default function App() {
  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden selection:bg-primary/25">
      ...
      {/* ═══ Desktop Sidebar ═══ */}
      <aside className="hidden lg:flex flex-col w-56 bg-background border-r border-border shrink-0">
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2.5">
                  <tab.icon className={cn("w-4 h-4", isActive ? "text-primary" : "text-text-ghost group-hover:text-text-muted")} />
                  {tab.label}
                </div>
                {tab.badge && (
                  <span className={cn(
                    "px-1.5 py-0.5 text-[9px] font-bold",
                    isActive ? "bg-primary/20 text-primary" : "bg-rose-500/15 text-rose-400"
                  )}>
                    {tab.badge}
                  </span>
                )}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {/* ── Topbar ── */}
        <header className="h-14 border-b border-border bg-background flex items-center px-4 sm:px-5 shrink-0 gap-4 z-20">
          {/* Mobile Logo */}
          ...
          <div className="hidden lg:flex items-center flex-1 max-w-xl gap-3">
            <div className="relative flex-1 group">
              ...
              <input 
                ...
                className="w-full bg-surface border border-border text-text-primary text-[12px] pl-9 pr-4 py-2 focus:outline-none focus:border-primary transition-colors placeholder:text-text-ghost"
              />
              <div className="absolute right-2 top-1/2 -translate-y-1/2">
                <span className="text-[9px] bg-surface border border-border text-text-ghost px-1.5 py-0.5 font-mono">⌘K</span>
              </div>
            </div>
            
            <div className="flex items-center gap-2 shrink-0">
              <span className="text-[10px] font-bold text-text-ghost uppercase tracking-[0.08em]">Período</span>
              <select className="bg-surface border border-border text-text-secondary text-[11px] font-medium px-2 py-1.5 focus:outline-none focus:border-primary cursor-pointer appearance-none">
          ...
          <div className="flex items-center ml-auto gap-2 shrink-0">
            ...
            <button 
              ...
              className={cn(
                "border px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.05em] transition-colors",
                demoMode 
                  ? "bg-primary/10 border-primary/30 text-primary"
                  : "bg-surface border-border text-text-ghost hover:text-text-muted"
              )}
            >
            ...
            <div className="relative">
              <button 
                ...
                className="relative p-2 text-text-muted hover:text-text-primary transition-colors hover:bg-surface"
              >
                <Bell className="w-4 h-4" />
                {notifications.length > 0 && (
                  <span className="absolute top-1 right-1 w-3.5 h-3.5 text-[8px] font-bold flex items-center justify-center bg-rose-500 text-white">
              ...
              {isNotifOpen && (
                <>
                  ...
                  <div className="absolute right-0 top-full mt-1 w-80 bg-card border border-border z-50 overflow-hidden animate-slide-up">
                      ...
                      {notifications.length > 0 ? notifications.map(n => (
                        <div key={n.id} className="px-4 py-3 border-b border-border last:border-none hover:bg-surface cursor-pointer transition-colors flex gap-3">
                          <div className={cn(
                            ...
                            n.type === 'warning' ? "bg-amber-500/10 text-amber-400" : "bg-primary/10 text-primary"
                          )}>
                            {n.type === 'error' ? <AlertCircle className="w-3.5 h-3.5" /> :
                             n.type === 'warning' ? <Workflow className="w-3.5 h-3.5" /> : <FileText className="w-3.5 h-3.5" />}
                          </div>
                          <div>
            ...
            <button 
              ...
              className="bg-primary hover:bg-primary-dark text-white pl-3 pr-4 py-2 text-[11px] font-bold transition-colors flex items-center gap-2 uppercase tracking-[0.03em]"
            >
        ...
        <nav className="hidden sm:flex lg:hidden bg-background border-b border-border px-1 overflow-x-auto hide-scrollbar">
          {tabs.map((tab) => (
            <button
              ...
              onClick={() => setActiveTab(tab.id)}
              className={cn(
                "flex items-center justify-center p-3 transition-colors border-b-2 whitespace-nowrap min-w-[50px]",
                activeTab === tab.id 
        ...
        <main className="flex-1 overflow-auto bg-background-alt relative">
          {!demoMode ? (
            <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-6">
              ...
              <h2 className="text-xl font-bold text-text-primary mb-2 tracking-tight">Portal Administrativo</h2>
              ...
              <button className="bg-primary hover:bg-primary-dark text-white font-bold px-8 py-3 text-sm transition-colors uppercase tracking-[0.05em]">
          ...
          {/* Footer */}
          <footer className="mt-auto border-t border-border bg-background px-5 py-6">
            <div className="max-w-[1900px] mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col">
                  <h4 className="text-text-primary text-[10px] font-bold uppercase tracking-[0.1em]">Igreja Batista Memorial</h4>
                  <p className="text-[9px] text-text-ghost">© 2024 Memorial Admin — Todos os direitos reservados.</p>
                </div>
              </div>
          ...
          {tabs.map((tab) => {
            ...
            return (
              ...
              >
                ...
                {tab.badge && !isActive && (
                  <span className="absolute top-1.5 right-1/4 w-1.5 h-1.5 bg-rose-500"></span>
                )}
              </button>
            );
          })}
